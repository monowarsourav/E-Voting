...
This property is the foundation of the proposed framework's $O(n)$-per-candidate (and $O(n \cdot m)$ total over $m$ candidates) tally. Given $n$ encrypted votes $\{E(v_1), E(v_2), \ldots, E(v_n)\}$ for a single candidate, the encrypted tally is:
\begin{equation}
...
This subsection describes the integration of the existing SA\textsuperscript{2} primitive of Apple Machine Learning Research~\cite{sa2_apple}---which itself builds on the secure aggregation construction of Bonawitz et al.~\cite{bonawitz2017}---into the proposed voting pipeline. \emph{The two-server, non-colluding mask-cancellation construction of SA\textsuperscript{2} is not a contribution of this thesis; it is invoked as an existing primitive.} The integration choice made here is to pair Paillier additive ciphertexts $E(v)$ with the SA\textsuperscript{2} mask-cancellation across the two aggregators in equations~\eqref{eq:sa2-share-a}--\eqref{eq:sa2-reconstruct}, so that the encrypted tally is reconstructed via mask homomorphism rather than via plaintext secret sharing (the original federated-learning setting).
