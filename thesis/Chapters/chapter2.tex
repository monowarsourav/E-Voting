...
Despite achieving scalable coercion resistance, the scheme has several limitations. It does not implement post-quantum cryptographic protection, leaving ballot privacy vulnerable to future quantum adversaries. The system lacks homomorphic encryption for privacy-preserving tallying, biometric voter authentication, and anonymous aggregation mechanisms. There is no integration with a production blockchain platform (such as Hyperledger Fabric) with deployed chaincode and real network benchmarks. The protocol implements only two cryptographic primitives, compared to the multi-protocol composition proposed in our framework. While the game-based formal proofs are stronger than symbolic-model verification, the system does not provide practical deployment validation or comprehensive test coverage.
...
A Pedersen commitment~\cite{pedersen1991} binds a committer to a secret value $m$ via $C = g^m h^r \bmod p$, where $g$ and $h$ are generators of a prime-order subgroup with $\log_g h$ unknown and $r$ is fresh randomness uniformly sampled from $\mathbb{Z}_q$. The construction is \emph{perfectly hiding} (the distribution of $C$ over uniform $r$ is independent of $m$, so even an unbounded adversary learns nothing about $m$ from $C$) and \emph{computationally binding} under the discrete logarithm assumption (a collision $g^m h^r = g^{m'} h^{r'}$ with $(m,r) \neq (m',r')$ would yield $\log_g h$). Pedersen commitments serve as the cryptographic substrate for the zero-knowledge proofs used to verify ballot well-formedness and for the deniable credentials that underpin coercion resistance in this work.
...
Table \ref{tab:comparison} presents a comparative analysis of the reviewed e-voting systems across key security properties and implementation characteristics. This comparison highlights the gaps in existing solutions that The proposed framework addresses through its multi-protocol cryptographic stack.
...
Among the works surveyed in this chapter, the proposed framework is the sole system that integrates all reviewed security properties (homomorphic encryption, zero-knowledge proofs, ring signatures, coercion resistance, post-quantum protection, biometric authentication, anonymous aggregation, and threshold decryption) into a single voting pipeline with empirical blockchain performance benchmarks. The proposed framework composes a \textbf{multi-protocol cryptographic stack} that combines substantially more primitives than the at-most-two integrated by any of the surveyed systems, and provides a full implementation with production blockchain deployment and ProVerif formal verification.
...
\begin{enumerate}
    ...
    \item \textbf{Protocol Integration Gap}: Existing systems implement at most two cryptographic protocols. The proposed framework integrates a multi-protocol cryptographic stack with substantially broader primitive composition while maintaining $O(n)$ tally complexity and sub-100ms per-vote pipeline time.
