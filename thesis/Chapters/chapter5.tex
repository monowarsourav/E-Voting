...
Our framework combines Paillier homomorphic encryption for encrypted tallying, Pedersen commitments with zero-knowledge $\Sigma$-proofs (Strong Fiat-Shamir) for vote validity verification, linkable ring signatures for anonymous ballot submission with double-vote detection, SMDC deniable credentials for coercion resistance, SA\textsuperscript{2} samplable anonymous aggregation for two-server vote privacy, Kyber768 hybrid encryption for post-quantum protection, and Damg\aa rd-Jurik-Nielsen threshold decryption for distributed key management. To the best of our knowledge, no existing system integrates this many cryptographic protocols in a single, functional voting pipeline.

We implemented the system in Go and deployed it on a production Hyperledger Fabric v2.5 blockchain network. Performance evaluation confirmed that the proposed framework achieves $O(n)$ tally complexity with a constant per-vote cost of approximately 6.1 $\mu$s, verified empirically from 100 to 100,000 voters. The complete seven-protocol vote casting pipeline processes a single vote in 70.5 ms. The Hyperledger Fabric network sustains approximately 200 TPS with 100\% success rate at 10,000 transactions, as measured by Hyperledger Caliper benchmarks. Post-quantum protection via Kyber768 adds only 26 $\mu$s of overhead, showing that quantum-resistant security is achievable at negligible computational cost.
...
\begin{itemize}
    ...
    \item \textbf{Lattice-Based Ring Signatures}: Our current linkable ring signatures are based on the discrete logarithm assumption, which is vulnerable to quantum attacks via Shor's algorithm~\cite{shor1997}. While Kyber768 protects the encryption layer, the ring signature scheme should be replaced with a lattice-based alternative (e.g., based on Module-SIS, the hardness assumption underlying NIST FIPS 204 ML-DSA~\cite{nist_mldsa}) to achieve full post-quantum security across all protocol components.
