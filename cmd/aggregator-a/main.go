...
// Server A for SA² aggregation
type AggregatorServerA struct {
	ServerID   string
	Port       string
...
func main() {
	...
	server.Aggregator = sa2.NewAggregator("ServerA", server.PublicKey)
	...
	log.Printf("✓ Server A ready at http://%s", addr)
	if err := router.Run(addr); err != nil {
		log.Fatalf("Failed to start server: %v", err)
	}
}

func (s *AggregatorServerA) healthCheck(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		"status":    "healthy",
		"server_id": s.ServerID,
		"role":      "SA² Aggregator Server A",
		"timestamp": time.Now().Unix(),
	})
}

func (s *AggregatorServerA) submitShare(c *gin.Context) {
	var req struct {
		ElectionID string `json:"election_id" binding:"required"`
		VoterID    string `json:"voter_id" binding:"required"`
		ShareA     string `json:"share_a" binding:"required"` // hex encoded big.Int
	}
	...
	log.Printf("Server A: Received share from voter %s for election %s", req.VoterID, req.ElectionID)
...
func (s *AggregatorServerA) aggregate(c *gin.Context) {
	var req struct {
		ElectionID string `json:"election_id" binding:"required"`
	}
...
func (s *AggregatorServerA) getResult(c *gin.Context) {
...
func (s *AggregatorServerA) clearShares(c *gin.Context) {
	electionID := c.Param("electionId")
	...
	log.Printf("Server A: Cleared shares for election %s", electionID)
